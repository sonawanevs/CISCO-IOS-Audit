...
#END - Completed all Layer 2 Switch checks
#----------------------------------------------------------------------------------------------------------------------


#START - Checks specific to Layer 3 Switch...... will be perfomed only if the Switch is Layer 3

		my (@interfaces, @open_interfaces, @valid_interfaces) = ();
		...
		if (($line =~ /\nrouter (rip|ergrp|begp|ospf)/i) || ($line =~ /\nip route.*/i) || ($#valid_interfaces > "-1"))
		{	
			# Perform layer 3 checks....
			my (@check20, @check21, @check22, @check25, @check19_v11, @check19_v12, @check29_absence, @check29_presence, @check29_access_list) = ();
			...
			if ($#valid_interfaces > "-1") {
				...
				$status = $status . "Status: Safe\n";
				$status = $status . "Safe Factor: All the interfaces are either shutdown or no interface is present.\n";
			...
			my $status7;
			$status7 = $status7 . "\n\n\nFollowing checks are not application for this device: \n";
			$status7 = $status7 . "Reason: Following checks are for Layer 3 Switch and this device is configured for Layer 2 operations :-)\n";
			$status7 = $status7 . "Check 10 : Insecure RIP Configuration    \n";
			$status7 = $status7 . "Check 11 : Insecure EIGRP configuration  \n";	
			$status7 = $status7 . "Check 12 : Insecure OSPF configuration  \n";
			$status7 = $status7 . "Check 13 : Insecure BGP configuration\n";
			$status7 = $status7 . "Check 18 : Device accepts IP source routed packets \n";
			$status7 = $status7 . "Check 19 : Device processes directed broadcasts \n";
			$status7 = $status7 . "Check 20 : Device sends IP unreachable messages\n";
			$status7 = $status7 . "Check 21 : Device sends ICMP mask-reply \n";
			$status7 = $status7 . "Check 22 : Device sends IP redirects\n";
			$status7 = $status7 . "Check 23 : UDP broadcast forwarding is enabled\n";
			$status7 = $status7 . "Check 25 : Proxy ARP is Not Disabled\n";
			$status7 = $status7 . "Check 29 : Anti-Spoof access control lists are not configured\n";
